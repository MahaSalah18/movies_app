import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryColor = Color(0xFFE50914); // الأحمر الأساسي (زرار Watch Now)
  static const Color backgroundColor = Color(0xFF000000); // الخلفية الأساسية (أسود)
  static const Color cardColor = Color(0xFF121212); // لون الكارد/البوكس
  static const Color secondaryColor = Color(0xFF1F1F1F); // خلفيات ثانوية
  static const Color textPrimary = Color(0xFFFFFFFF); // النص الأساسي (أبيض)
  static const Color textSecondary = Color(0xFFB3B3B3); // نص رمادي فاتح
  static const Color borderColor = Color(0xFF2A2A2A); // حدود خفيفة
  static const Color ratingColor = Color(0xFFFFC107); // لون التقييم (أصفر)
  static const Color buttonSecondary = Color(0xFF2C2C2C); // زرار More Info
  static const Color  errorBuilder= Color(0xFF1E1E1E);
  static const Color iconColor=  Colors.white38;
    static const Color white70=  Colors.white70;
}
